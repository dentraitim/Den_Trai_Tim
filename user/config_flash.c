...
void ICACHE_FLASH_ATTR config_load_default(sysconfig_p config) {
  ...
  if (reg3 != 0) {
    ...
    mac[1] = (reg3 >> 8) & 0xff;
    mac[2] = reg3 & 0xff;
  } else if (((reg1 >> 16) & 0xff) == 0) {
    mac[0] = 0x1a;
    mac[1] = 0xfe;
    mac[2] = 0x34;
  } else if (((reg1 >> 16) & 0xff) == 1) {
    mac[0] = 0xac;
    mac[1] = 0xd0;
    mac[2] = 0x74;
  } else {
...
#ifdef MQTT_CLIENT
  os_sprintf(config->mqtt_host,"%s", "test.mosquitto.org");
  config->mqtt_port = 1883;
  os_sprintf(config->mqtt_user,"%s", "none");
  config->mqtt_password[0] = 0;
  os_sprintf(config->mqtt_id,"%s_%02x%02x%02x", MQTT_ID_PREFIX, mac[3], mac[4], mac[5]);
  os_sprintf(config->mqtt_pub_topic,"%s/%s/%s", MQTT_TOPIC_PREFIX, config->mqtt_id, "led");
  os_sprintf(config->mqtt_sub_topic,"%s/%s/%s", MQTT_TOPIC_PREFIX, "+", "led");
#endif
